...
// Groq key is kept safely in the backend .env as GROQ_API_KEY
const AUTH_USER_KEY = "agrimind_backend_user_v1";
const AUTH_TOKEN_KEY = "agrimind_backend_token_v1";
const LEGACY_AUTH_KEYS = [
  "agrimind_auth_users_v3",
  "agrimind_auth_session_v3",
  "agrimind_current_user",
  "agrimind_auth_session",
  "agrimind_auth_users",
];
...
function getInitialAuthMode() {
  ...
  return path.includes("signup") || hash.includes("signup") ? "signup" : "login";
}

function isForcedAuthRoute() {
  const path = window.location.pathname.toLowerCase();
  const hash = window.location.hash.toLowerCase();
  return path.includes("login") || path.includes("signup") || hash.includes("login") || hash.includes("signup");
}
...
async function readGroqLikeResponse(res, setStreamText) {
  ...
  // Backend normally returns JSON. This also supports SSE streaming if added later.
...
export default function AgriAssistant() {
  ...
  const [authUser, setAuthUser] = useState(() => {
    if (isForcedAuthRoute()) return null;
  ...
  const saveKey = () => {
    // API key is handled by backend .env. This modal is disabled by default.
  ...
  const changeAuthMode = (mode) => {
    setAuthMode(mode);
    setAuthError("");
    window.history.pushState(null, "", mode === "signup" ? "/signup" : "/login");
  };
  ...
  const handleLogin = async ({ email, password }) => {
    ...
    try {
      ...
      setAuthError(err.message || "Login failed. Please create an account first.");
    }
  };
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({
          model: selectedModel,
          system: buildSystem(newMessages),
          messages: apiMsgs,
          temperature: 0.7,
          max_tokens: 2048,
        })
      ...
      const errMsg = err.message.includes("401")
        ...
        ? "❌ **Backend API key missing.** Add GROQ_API_KEY in your .env locally and in Vercel Environment Variables."
        : `❌ **Error:** ${err.message}`;
      ...
      {showKeyModal && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
          <div style={{ background:C.modalBg, borderRadius:18, padding:28, width:"100%", maxWidth:420, border:`2px solid ${C.accent}` }}>
            ...
            <p style={{ textAlign:"center", color:C.muted, fontSize:13, margin:"0 0 18px" }}>Powered by <strong>Backend + Groq</strong></p>
            <div style={{ background:darkMode?"#0f2a17":"#e8f5eb", borderRadius:10, padding:"12px 14px", marginBottom:16, fontSize:13, color:C.muted, lineHeight:1.7 }}>
              <strong style={{ color:C.accentDark }}>Backend key setup:</strong><br/>
              Add <strong>GROQ_API_KEY</strong> in your backend .env / Vercel Environment Variables ✅
            </div>
            ...
            <p style={{ textAlign:"center", fontSize:11, color:C.muted, marginTop:10 }}>API key is never exposed in browser</p>
          </div>
        </div>
      )}
      ...
      {sidebarOpen && (
        <div style={{
          ...
          <div style={{ padding:"10px 10px 16px", borderTop:`1px solid ${C.border}` }}>
            <div style={{ padding:"9px 10px", borderRadius:10, background:C.chip, border:`1px solid ${C.chipBorder}`, color:C.text, fontSize:12, marginBottom:7 }}>
              <div style={{ fontWeight:700, display:"flex", alignItems:"center", gap:6 }}>👤 {authUser.name}</div>
              <div style={{ color:C.muted, fontSize:10, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{authUser.email}</div>
            </div>
...
function AuthScreen({ mode, error, onModeChange, onLogin, onSignup }) {
  ...
  return (
    <div style={{ minHeight:"100dvh", background:`radial-gradient(circle at top left, ${C.bg2}, ${C.bg1} 55%)`, display:"flex", alignItems:"center", justifyContent:"center", padding:18, fontFamily:"'Segoe UI',system-ui,sans-serif" }}>
      <div style={{ position:"absolute", inset:0, opacity:0.15, backgroundImage:"linear-gradient(45deg, transparent 48%, #95d5a6 49%, #95d5a6 51%, transparent 52%)", backgroundSize:"38px 38px" }} />
      <div style={{ width:"100%", maxWidth:430, position:"relative", zIndex:1 }}>
        <div style={{ textAlign:"center", marginBottom:18 }}>
          <div style={{ width:76, height:76, margin:"0 auto 12px", borderRadius:24, background:"linear-gradient(135deg,#52b788,#1b4332)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:42, boxShadow:"0 16px 45px rgba(0,0,0,0.35)" }}>🌾</div>
          <h1 style={{ margin:0, color:"#fff", fontSize:30, fontWeight:900, letterSpacing:-0.5 }}>AgriMind</h1>
          <p style={{ margin:"6px 0 0", color:"rgba(255,255,255,0.72)", fontSize:14 }}>AI Agriculture Advisor for farmers and students</p>
        </div>

        <form onSubmit={submit} style={{ background:C.card, borderRadius:24, padding:24, boxShadow:"0 25px 70px rgba(0,0,0,0.35)", border:`1px solid ${C.border}` }}>
          <div style={{ display:"flex", background:"#e8f5eb", borderRadius:14, padding:4, marginBottom:18 }}>
            <button type="button" onClick={() => onModeChange("login")} style={{ flex:1, padding:"10px", border:"none", borderRadius:11, cursor:"pointer", fontWeight:800, background:!isSignup?C.accent:"transparent", color:!isSignup?"#fff":C.muted }}>Login</button>
            <button type="button" onClick={() => onModeChange("signup")} style={{ flex:1, padding:"10px", border:"none", borderRadius:11, cursor:"pointer", fontWeight:800, background:isSignup?C.accent:"transparent", color:isSignup?"#fff":C.muted }}>Sign Up</button>
          </div>

          <h2 style={{ margin:"0 0 5px", color:C.text, fontSize:22 }}>{isSignup ? "Create your account" : "Welcome back"}</h2>
          <p style={{ margin:"0 0 18px", color:C.muted, fontSize:13 }}>{isSignup ? "Sign up to start using AgriMind." : "Login to open your agriculture assistant."}</p>

          {isSignup && (
            <div style={{ marginBottom:12 }}>
              <label style={{ display:"block", fontSize:12, fontWeight:800, color:C.muted, marginBottom:6 }}>Full Name</label>
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Talha Tariq" autoComplete="name" style={{ width:"100%", padding:"13px 14px", borderRadius:12, border:`1.5px solid ${C.border}`, outline:"none", fontSize:14, color:C.text, boxSizing:"border-box" }} />
            </div>
          )}

          <div style={{ marginBottom:12 }}>
            <label style={{ display:"block", fontSize:12, fontWeight:800, color:C.muted, marginBottom:6 }}>Email</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" autoComplete="email" style={{ width:"100%", padding:"13px 14px", borderRadius:12, border:`1.5px solid ${C.border}`, outline:"none", fontSize:14, color:C.text, boxSizing:"border-box" }} />
          </div>

          <div style={{ marginBottom:12 }}>
            <label style={{ display:"block", fontSize:12, fontWeight:800, color:C.muted, marginBottom:6 }}>Password</label>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Minimum 6 characters" autoComplete={isSignup ? "new-password" : "current-password"} style={{ width:"100%", padding:"13px 14px", borderRadius:12, border:`1.5px solid ${C.border}`, outline:"none", fontSize:14, color:C.text, boxSizing:"border-box" }} />
          </div>

          {error && <div style={{ background:"#fff3cd", border:"1px solid #ffe08a", color:"#8a5a00", padding:"10px 12px", borderRadius:10, fontSize:12, marginBottom:12 }}>{error}</div>}

          <button type="submit" style={{ width:"100%", padding:"14px", borderRadius:13, background:C.accent, border:"none", color:"#fff", fontSize:15, fontWeight:900, cursor:"pointer", boxShadow:"0 10px 25px rgba(45,106,79,0.3)" }}>
            {isSignup ? "Create Account" : "Login"} 🚀
          </button>

          <p style={{ textAlign:"center", margin:"14px 0 0", color:C.muted, fontSize:12 }}>
            {isSignup ? "Already have an account?" : "New user?"} {" "}
            <button type="button" onClick={() => onModeChange(isSignup ? "login" : "signup")} style={{ border:"none", background:"transparent", color:C.accent, fontWeight:900, cursor:"pointer", padding:0 }}>
              {isSignup ? "Login" : "Create account"}
            </button>
          </p>
        </form>

        <p style={{ textAlign:"center", color:"rgba(255,255,255,0.58)", fontSize:11, marginTop:14 }}>
          Backend auth + Groq proxy enabled. Add a database later for permanent public accounts.
        </p>
      </div>
    </div>
  );
